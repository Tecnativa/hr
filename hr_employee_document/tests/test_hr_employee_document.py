...
class TestHrEmployeeDocument(common.TransactionCase):
    def setUp(self):
        ...
        self.user_1 = new_test_user(self.env, login="Test user employee 1",)
        self.user_2 = new_test_user(self.env, login="Test user employee 2",)
        self.user_manager = new_test_user(
            self.env, login="Test user manager", groups="hr.group_hr_user",
        )
        self.employee_1 = (
